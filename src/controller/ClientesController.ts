...
class ClienteController extends Cliente {
  static async todos(req: Request, res: Response): Promise<Response> {
    try {
      const listaClientes: Array<Cliente> | null =
  ...
  static async novo(req: Request, res: Response): Promise<Response> {
    try {
      ...
      const respostaModelo = await Cliente.cadastrarCliente(
      ...
      return res
        .status(500)
        .json({ mensagem: "Não foi possivel inserir o cliente." });
    }
  }
  ...
  static async cliente(req: Request, res: Response): Promise<Response> {
    try {
      ...
      if (respostaModelo === null) {
        return res
          .status(200)
          .json({ mensagem: "Nenhum cliente encontrado com esse cpf." });
      }
      ...
      return res
        .status(500)
        .json({ mensagem: "Não foi possivel recuperar o cliente" });
    }
  }
}
